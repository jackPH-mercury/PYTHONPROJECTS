...
class CoffeeVendingMachine(vm.VendingMachine):
    ...
    def set_cream(self,count):
        if self.balance < self.item_price:
            print("Please insert sufficient funds before making selections.")
            return
        
        if 0 <= count <= 2:
            self.cream = count
            print(f"Cream set to {count}.")
        else:
            print("Invalid cream amount. Enter a value between 0 and 2.")
    ...
    def set_sugar(self, count):
        ...
        if 0 <= count <= 2:
            self.sugar = count
            print(f"Sugar set to {count}.")
        else:
            print("Invalid sugar amount. Enter a value between 0 and 2.")
    ...
    def set_strength(self, count):
        ...
        if 1 <= count <= 3:
            ...
            print("Invalid strength. Enter a value between 1 and 3.")
...
if __name__ == "__main__":
    machine = CoffeeVendingMachine("Coffee", 1.50, 10)
    machine.menu()